...
library(GSEABase)
library(DESeq2)
#################################################### Create DESeq2 object for all samples #################################################

combined_matrix_all <- cbind(
  normal_weight_matrix,
  over_weight_matrix,
  obese_matrix
)

bmi_group <- c(
  rep("normal_weight", ncol(normal_weight_matrix)),
  rep("over_weight",   ncol(over_weight_matrix)),
  rep("obese",         ncol(obese_matrix))
)

sample_info_all <- data.frame(
  BMI_group = factor(bmi_group, levels = c("normal_weight", "over_weight", "obese"))
)

rownames(sample_info_all) <- colnames(combined_matrix_all)

dds_all <- DESeqDataSetFromMatrix(
  countData = round(combined_matrix_all),
  colData   = sample_info_all,
  design    = ~ BMI_group
)

# Load custom ImmPort gene set from GMT
gmt_file <- "C:/Users/arunv/OneDrive/Desktop/CPTAC analysis 06-04-2025/immport_signature/immport_signature.gmt"
...
run_custom_gmt_ssgsea(
  dds_object = dds_all,
  gmt_path = "C:/Users/arunv/OneDrive/Desktop/CPTAC analysis 06-04-2025/immport_signature/immport_signature.gmt",
